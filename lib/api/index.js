...
function createApp(stumble, conf) {
  ...
  router.get('/player', function (req, res, next) {
    function currentState() {
      const gainFloat = stumble.execute('audio::gain', {})
      const gain = ~~(gainFloat * 100)
      const streaming = stumble.space.get('audio.streaming')
      const queue = stumble.space.get('audioplayer.queue')
        .map(sound => sound.key)
      return { gain, streaming, queue }
    }

    if (!req.query.repeating) {
    ...
    tick()
    let timer
    // Be careful to set this handler in this (synchronous) function
    // to ensure we never miss the "close" event
