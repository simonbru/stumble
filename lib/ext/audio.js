...
const delayPipe = require('delay-stream');
  ...
  exec: function playfile (data) {
    ...
    ffmpeg.ffprobe(data.filename, (error, meta) => {
      ...
      if (!this.io.input) {
        ...
        const samples = 48000;
        const gain = (data.gain == undefined) ? conf.gain : data.gain
        ...
        this.io.establish({
          ...
          inputOptions: {
            ...
            sampleRate: samples,
            gain: minmax(0.01, gain, 1.0)
          },
          inputTargets: data.targetSessions,
        });
...
const stopfile = {
  ...
  exec: function stopfile (data) {
    if (this.space.get('audio.streaming')) {
      ...
      if (force) this.io.input.setGain(0.01);

      pass.unpipe(this.io.input);

      if (force) pass.emit('kill');
      else pass.end();

      this.io.nullify({ input: true, output: false });
...
const feedback = {
  ...
  exec: function feedback (data) {
    const delay = +data.message.trim()
    ...
    if (feeding) {
      ...
      if (isNaN(delay)) {
        ...
        this.io.output.pipe(delayPipe(delay)).pipe(this.io.input);
      }
    }
  },
  commands: [{
    handle: 'feedback',
    exec: function feedback (data) {
      this.execute('audio::feedback', data);
    },
    info: () => `<pre>
USAGE: feedback
