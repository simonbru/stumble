...
function Greeter(stumble, greetings = GREETINGS, yall = YALL) {
  ...
  async function greet(user) {
    ...
    let target;
    if (user == undefined) {
      target = randomPick(yall)
    } else {
      const userprefs = await db.getAsync(
        'SELECT * FROM greeterprefs WHERE name = ?', user.name
      )
      if (userprefs && userprefs.ignore) return
      target = user.name
    }
    const message = `${greet} ${target} !`
    console.log(message)
    stumble.invoke('tts', { message })
  }
